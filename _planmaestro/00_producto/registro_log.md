...
| ⚪ | 22 | **`/api/prueba` quedó apuntando a una tabla que ya no existe en pruebas.** `functions/api/prueba.js` consulta `prueba_tuberia`, y el vaciado del ensayo remoto la retiró de la base de pruebas: contra el entorno de vista previa ese extremo ya falla. **No rompe ninguna página** —`leerPrueba()` está exportado en `static/js/servicios/datos.js` y no lo llama ningún componente—, pero el extremo de juguete tiene que retirarse junto con su tabla, o dejar de ser de juguete. Le toca a esta iteración, que es la que rehace los extremos de lectura |
